...
async function processDate(dStr, isoDate) {
  ...
  groups.forEach((g) => {
    ...
    matchesList.forEach((m) => {
      if (m && m[1] && m[3]) {
        ...
        // m[14] = İY Skoru (örn "1-0"), m[15] = MS Skoru (örn "2-1")
        const iyScore = cleanScoreString(m[14]);
        const msScore = cleanScoreString(m[15]);
  ...
  // İlk 10 maçı örnek göster
  console.log(`\n--- Geniş İddaa Örnek Maçlar (${dStr}) ---`);
  parsedMatches.slice(0, 10).forEach((m, idx) => {
    console.log(`${idx + 1}. [${m.league}] ${m.home_team} ${m.ms_score || 'Sonuçlanmadı/Bülten'} ${m.away_team} (İY: ${m.iy_score || '-'}) | MS: ${m.ms_1_odd} - ${m.ms_0_odd} - ${m.ms_2_odd} | 2.5 Alt/Üst: ${m.alt_25_odd} / ${m.ust_25_odd}`);
  });
}
