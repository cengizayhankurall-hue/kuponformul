...
async function processDate(dStr, isoDate) {
  ...
  // 1. Delete existing records for isoDate so we have fresh updated scores
  ...
  if (status !== 200) {
    console.warn(`HTTP ${status} hatasi alindi.`);
    return 0;
  }
...
async function run() {
  const dates = [
    { dStr: '13/08/2026', isoDate: '2026-08-13' },
    { dStr: '14/08/2026', isoDate: '2026-08-14' },
    { dStr: '15/08/2026', isoDate: '2026-08-15' },
    { dStr: '16/08/2026', isoDate: '2026-08-16' },
    { dStr: '17/08/2026', isoDate: '2026-08-17' },
    { dStr: '18/08/2026', isoDate: '2026-08-18' }
  ];

  console.log(`Toplam ${dates.length} gunluk mac sonuclari ve oranlari Supabase'e aktarilacak...`);
  let grandTotal = 0;

  for (const item of dates) {
    try {
      const count = await processDate(item.dStr, item.isoDate);
      grandTotal += count;
    } catch (e) {
      console.error(`[HATA] ${item.dStr} islenirken hata:`, e.message);
    }
  }

  console.log(`\n==================================================`);
  console.log(`TUM ISLEM BASARIYLA TAMAMLANDI! Toplam ${grandTotal} yeni/guncel mac Supabase veritabanina eklendi.`);
  console.log(`==================================================`);
}
