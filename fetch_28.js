...
async function fetchAndUpload() {
  console.log('Fetching data for 28.07.2026 from Mackolik...');
  const d = '28.07.2026';
  ...
  groups.forEach((g) => {
    ...
    matchesList.forEach((m) => {
      if (m[1] && m[3]) {
        ...
        const status = String(m[5] || m[10] || m[9] || '');
        ...
        if (hMS !== null && hMS !== undefined && aMS !== null && aMS !== undefined) {
           msScore = `${hMS}-${aMS}`;
        }
        if (hIY !== null && hIY !== undefined && aIY !== null && aIY !== undefined) {
           iyScore = `${hIY}-${aIY}`;
        }
  ...
  console.log(`Found ${parsedMatches.length} matches. Upserting into past_matches...`);
  ...
  console.log('Done! All results added/updated.');
}

fetchAndUpload().catch(console.error);
