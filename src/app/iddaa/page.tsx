...
import { TURNAROUND_MATCHES, getTurnaroundStats, TurnaroundMatch } from '@/lib/turnaroundMatchesData';
...
export default function IddaaPage() {
  ...
  // Main Navigation Tab
  const [activeMainTab, setActiveMainTab] = useState<'bulletin' | 'turnaround'>('bulletin');

  // Turnaround Matches State & Filters (Son 7 Ay 1/2 ve 2/1 Maçları)
  const [turnaroundType, setTurnaroundType] = useState<'all' | '1/2' | '2/1'>('all');
  ...
  const turnaroundAvailableLeagues = useMemo(() => {
    return ['Tümü', ...turnaroundStats.leagues];
  }, [turnaroundStats]);

  const turnaroundAvailableMonths = useMemo(() => {
    return ['Tümü', ...turnaroundStats.months];
  }, [turnaroundStats]);

  const filteredTurnarounds = useMemo(() => {
    let list = [...TURNAROUND_MATCHES];
    if (turnaroundType !== 'all') {
      list = list.filter(m => m.type === turnaroundType);
    }
    if (turnaroundMonth !== 'Tümü') {
      list = list.filter(m => m.month === turnaroundMonth);
    }
    if (turnaroundLeague !== 'Tümü') {
      list = list.filter(m => m.league === turnaroundLeague);
    }
    if (turnaroundSearch.trim()) {
      const q = turnaroundSearch.toLowerCase().trim();
      list = list.filter(m =>
  ...
  return (
    <div className={`min-h-[calc(100vh-64px)] font-sans pb-16 flex flex-col transition-colors duration-200 ${isDark ? 'bg-[#0b0f19] text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
        ...
        {/* TOP NAVIGATION TABS (BÜLTEN & 1/2 - 2/1 ÇEVİREN MAÇLAR) */}
        <div className="flex items-center gap-3 mb-6 overflow-x-auto pb-1 scrollbar-none">
          <button
            onClick={() => setActiveMainTab('bulletin')}
            className={`px-5 py-3 rounded-2xl font-black text-xs md:text-sm flex items-center gap-2.5 transition-all shadow-sm cursor-pointer whitespace-nowrap ${
              activeMainTab === 'bulletin'
                ? 'bg-gradient-to-r from-sky-500 to-blue-600 text-white shadow-lg shadow-sky-500/20 scale-[1.02]'
                : isDark
          ...
          >
            ...
            <span>📋 Güncel İddaa Bülteni</span>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
              activeMainTab === 'bulletin' ? 'bg-white/20 text-white' : 'bg-slate-500/10 text-slate-500'
            }`}>
              {matches.length} Maç
            </span>
          </button>
          ...
          <button
            onClick={() => setActiveMainTab('turnaround')}
            className={`px-5 py-3 rounded-2xl font-black text-xs md:text-sm flex items-center gap-2.5 transition-all shadow-sm cursor-pointer whitespace-nowrap relative ${
              activeMainTab === 'turnaround'
          ...
          >
            ...
            <span>🔄 1/2 ve 2/1 Çeviren Maçlar</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-rose-600 text-white shadow-sm animate-pulse">
              Son 7 Ay
            </span>
          </button>
        </div>
        ...
        {activeMainTab === 'turnaround' ? (
          /* ========================================================================= */
          /* 1/2 VE 2/1 ÇEVİREN MAÇLAR (SON 7 AY ARŞİVİ & ORAN ANALİZİ)                */
          /* ========================================================================= */
          <div className="space-y-6 animate-in fade-in duration-300">
            
            {/* Turnaround Header */}
            <div className={`p-6 rounded-2xl border transition-colors ${isDark ? 'bg-slate-900/30 border-slate-800/60' : 'bg-white border-slate-200/80 shadow-sm'}`}>
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-gradient-to-r from-amber-400 to-orange-500 text-black uppercase tracking-wider flex items-center gap-1 shadow-lg shadow-amber-500/10">
                      <Flame className="h-3 w-3" />
                      Son 7 Ay İstatistik Arşivi (Şubat 2026 - Eylül 2026)
                    </span>
                  ...
                  <h1 className={`text-2xl md:text-3xl font-black tracking-tight mt-2 ${isDark ? 'text-white' : 'text-slate-950'}`}>
                    1'den 2 ve 2'den 1 Biten Maçlar & Oranları
                  </h1>
                  <p className={`text-xs md:text-sm mt-1 max-w-3xl ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    İlk yarıyı önde kapatıp maçı kaybeden (1/2) veya ilk yarı geriye düşüp maçı kazanan (2/1) tüm maçların ilk yarı / maç sonu skorları, açılan İddaa oranları ve lig bazlı dönüşüm analizleri.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mt-6">
                <div className={`p-4 rounded-xl border ${isDark ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-50 border-slate-200/80 shadow-inner'}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-400">Toplam Çeviren Maç</span>
                    <BarChart3 className="w-4 h-4 text-amber-400" />
                  </div>
                  <div className="text-2xl font-black mt-2 text-amber-400">{turnaroundStats.total} Maç</div>
                  <div className="text-[10px] text-slate-500 mt-0.5">Son 7 Ayda 1/2 ve 2/1 Bitenler</div>
                </div>

                <div className={`p-4 rounded-xl border ${isDark ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-50 border-slate-200/80 shadow-inner'}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-400">1'den 2 (İY 1 / MS 2)</span>
                    <span className="px-1.5 py-0.5 text-[9px] font-black rounded bg-emerald-500/20 text-emerald-400">1/2</span>
                  </div>
                  <div className="text-2xl font-black mt-2 text-emerald-400">{turnaroundStats.count1to2} Maç</div>
                  <div className="text-[10px] text-slate-500 mt-0.5">Toplamın %{Math.round((turnaroundStats.count1to2 / turnaroundStats.total) * 100)}'i</div>
                </div>

                <div className={`p-4 rounded-xl border ${isDark ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-50 border-slate-200/80 shadow-inner'}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-purple-400">2'den 1 (İY 2 / MS 1)</span>
                    <span className="px-1.5 py-0.5 text-[9px] font-black rounded bg-purple-500/20 text-purple-400">2/1</span>
                  </div>
                  <div className="text-2xl font-black mt-2 text-purple-400">{turnaroundStats.count2to1} Maç</div>
                  <div className="text-[10px] text-slate-500 mt-0.5">Toplamın %{Math.round((turnaroundStats.count2to1 / turnaroundStats.total) * 100)}'i</div>
                </div>

                <div className={`p-4 rounded-xl border ${isDark ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-50 border-slate-200/80 shadow-inner'}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-rose-400">Ortalama 1/2 & 2/1 Oranı</span>
                    <Zap className="w-4 h-4 text-rose-400" />
                  </div>
                  <div className="text-2xl font-black mt-2 text-rose-400">{turnaroundStats.avgOdds} Oran</div>
                  <div className="text-[10px] text-slate-500 mt-0.5">En Çok: {turnaroundStats.topLeague.league} ({turnaroundStats.topLeague.count})</div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-grow">
                  ...
                  <input
                    type="text"
                    placeholder="Takım adı veya lig ara (Örn: Galatasaray, Beşiktaş, Premier League)..."
                    value={turnaroundSearch}
                    onChange={(e) => setTurnaroundSearch(e.target.value)}
                    className={`w-full border rounded-xl py-3 pl-10 pr-10 text-sm transition shadow-inner ${
                      isDark 
                        ? 'bg-[#111827]/80 border-slate-800 text-slate-200 placeholder-slate-500 focus:border-amber-500/50 focus:ring-amber-500/30' 
                        : 'bg-white border-slate-200 text-slate-850 placeholder-slate-400 focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/10'
                    }`}
                  />
                  {turnaroundSearch && (
                    <button
                      onClick={() => setTurnaroundSearch('')}
              ...
              {/* Type Filter Pills (Tümü / 1'den 2 / 2'den 1) */}
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                {[
                  { id: 'all', label: 'Tümü', count: turnaroundStats.total, color: 'sky' },
                  { id: '1/2', label: '🟢 Sadece 1\'den 2 (İY 1 / MS 2)', count: turnaroundStats.count1to2, color: 'emerald' },
                  { id: '2/1', label: '🟣 Sadece 2\'den 1 (İY 2 / MS 1)', count: turnaroundStats.count2to1, color: 'purple' }
                ].map(item => (
                  <button
                    key={item.id}
                    onClick={() => setTurnaroundType(item.id as any)}
                  ...
                  >
                    ...
                    <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${
                      turnaroundType === item.id ? 'bg-white/20 text-white' : 'bg-slate-500/10 text-slate-500'
                    }`}>
                      {item.count}
                    </span>
                  </button>
              ...
              {/* Month Filter Pills (Son 7 Ay) */}
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                <span className="text-xs font-bold text-slate-500 self-center mr-1">Ay:</span>
                {turnaroundAvailableMonths.map(m => (
                  <button
                    key={m}
                    onClick={() => setTurnaroundMonth(m)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap border transition cursor-pointer flex items-center gap-1.5 ${
                      turnaroundMonth === m
                        ? 'bg-amber-500/15 text-amber-400 border-amber-500/40 font-black shadow-sm'
                        : isDark
                          ? 'bg-slate-900/40 text-slate-400 border-slate-800 hover:bg-slate-800/30 hover:text-slate-200'
                          : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100 shadow-sm'
                    }`}
                  >
                    {m !== 'Tümü' && <Calendar className="w-3 h-3 opacity-70" />}
                    {m}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                <span className="text-xs font-bold text-slate-500 self-center mr-1">Lig:</span>
                {turnaroundAvailableLeagues.map(l => (
                  <button
                    key={l}
                    onClick={() => setTurnaroundLeague(l)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap border transition cursor-pointer flex items-center gap-1.5 ${
                      turnaroundLeague === l
                        ? 'bg-sky-500/15 text-sky-400 border-sky-500/40 font-black shadow-sm'
                        : isDark
                          ? 'bg-slate-900/40 text-slate-400 border-slate-800 hover:bg-slate-800/30 hover:text-slate-200'
                          : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100 shadow-sm'
                    }`}
                  >
                    {turnaroundLeague === l && <div className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" />}
                    {l}
                  </button>
                ))}
              </div>
            ...
            {filteredTurnarounds.length > 0 ? (
              ...
              <div className={`text-center py-16 rounded-2xl border ${isDark ? 'bg-slate-900/30 border-slate-800' : 'bg-white border-slate-200'}`}>
                ...
                <h4 className="text-base font-bold text-slate-400">Filtrelere Uygun Maç Bulunamadı</h4>
                <p className="text-xs text-slate-500 mt-1">Lütfen lig veya ay filtrelerini genişleterek tekrar deneyin.</p>
              </div>
            )}
          ...
          /* ========================================================================= */
          /* GÜNCEL İDDAA BÜLTENİ (MEVCUT YAPI AYDINLIK / KOYU TEMASI İLE KORUNUYOR)     */
          /* ========================================================================= */
          <>
        {/* HEADER SECTION */}
