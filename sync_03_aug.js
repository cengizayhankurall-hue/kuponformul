    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if ((res.statusCode === 502 || res.statusCode === 503) && retries > 0) {
          console.warn(`HTTP ${res.statusCode} alındı, 1.5 sn sonra tekrar deneniyor... (${retries} hakkı kaldı)`);
          setTimeout(() => {
...
async function run() {
  const dStr = '03/08/2026';
  const isoDate = '2026-08-03';

  await processDate(dStr, isoDate);

  console.log(`\n==================================================`);
  console.log(`VERİTABANI DOĞRULAMA KONTROLÜ (03/08/2026)`);
  console.log(`==================================================`);
  ...
  console.log(`Tarih: ${dStr} (${isoDate}) -> Supabase Kayıt Sayısı: ${count}`);
